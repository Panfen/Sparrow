...
export function rect(context, attributes) {
   ...
   * 错误：<rect width="-60" height="-60" x="100" y="100" />
   * 为了使其支持负数的 width 和 height，我们转换成如下的形式:
   * 正确：<rect width="60" height="60" x="40" y="40" />
   */
...
export function circle(content, attributes) {
  return shape('circle', content, attributes);
}

/**
 * text 元素是将展示内容放在标签内部，而不是作为标签的属性
 * 错误：<text text='content' />
 * 正确：<text>content</text>
 */
